...
func NewCmd(streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `kubectl-kenv is a kubectl plugin that extracts environment variables from Kubernetes manifests
and formats them for use with docker run or shell commands.

Examples:
  # Extract env vars from a deployment
  kubectl kenv extract deployment/my-app

  # Extract env vars from a specific container
  kubectl kenv extract deployment/my-app -c my-container

  # Output in docker run format
  kubectl kenv extract deployment/my-app --format docker`,
		Version: version,
	}

	o.configFlags.AddFlags(cmd.Flags())

	cmd.AddCommand(newExtractCmd(o))

	return cmd
}

func newExtractCmd(o *Options) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "extract [TYPE/NAME]",
		Short: "Extract environment variables from Kubernetes resources",
		Long: `Extract environment variables from Kubernetes resources.
